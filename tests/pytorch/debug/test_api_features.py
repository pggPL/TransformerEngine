...
def test_transformer_engine_no_config(feature_dirs):
    ...
    try:
        ...
        output = nvinspect_api.transformer_engine.process_tensor("decoder.1.attn.qkv", tensor=tensor, gemm="fprop", tensor_name="activation")
        assert (output == tensor).all()

        # use process tensor - False by default
        assert not nvinspect_api.transformer_engine.use_process_tensor("decoder.1.attn.qkv", tensor=tensor, gemm="fprop", tensor_name="activation")

        tensor_copy = tensor.clone()

        # save tensor for stats - it does nothing
        nvinspect_api.transformer_engine.look_at_tensor_before_process("decoder.1.attn.qkv", tensor=tensor, gemm="fprop", tensor_name="activation")
        assert (tensor_copy == tensor).all()

        # save tensor for stats - it does nothing
        nvinspect_api.transformer_engine.look_at_tensor_after_the_process("decoder.1.attn.qkv", tensor=tensor, gemm="fprop", tensor_name="activation")
        assert (tensor_copy == tensor).all()
...
def test_per_tensor_scaling(configs_dir, feature_dirs):
    ...
    try:
        ...
        default_quantizer1 = Float8Quantizer(scale=torch.tensor([1]).cuda(), amax=torch.tensor([0]).cuda(), fp8_dtype=tex.DType.kFloat8E4M3)
        default_quantizer2 = Float8Quantizer(scale=torch.tensor([1]).cuda(), amax=torch.tensor([0]).cuda(), fp8_dtype=tex.DType.kFloat8E5M2)

        output1 = nvinspect_api.transformer_engine.process_tensor(
            "decoder.1.mlp.fc1", gemm="fprop", tensor=tensor, tensor_name="activation", default_quantizer=default_quantizer1)
        ...
        output2 = nvinspect_api.transformer_engine.process_tensor(
            "decoder.1.mlp.fc1", gemm="dgrad", tensor=tensor, tensor_name="gradient", default_quantizer=default_quantizer2)
        ...
        output3 = nvinspect_api.transformer_engine.process_tensor(
            "decoder.1.mlp.fc1", gemm="wgrad", tensor=tensor, tensor_name="gradient", default_quantizer=default_quantizer2)
        ...
        nvinspect_api.transformer_engine.process_tensor("decoder.1.mlp.fc4", gemm="fprop", tensor=tensor, tensor_name="activation", fp8_dtype=tex.DType.kFloat8E4M3)
...
def test_fake_quant(configs_dir, feature_dirs):
    ...
    try:
        ...
        # use_process_tensor
        nvinspect_api.transformer_engine.use_process_tensor("decoder.1.mlp.fc1", gemm="fprop", tensor_name="activation")
        #assert check_debug_log("decoder.1.mlp.fc1: Feature=FakeQuantFp8, API=process_tensor: fprop, activation: QDQ")

        nvinspect_api.transformer_engine.use_process_tensor("decoder.1.mlp.fc1", gemm="dgrad", tensor_name="gradient")
        #assert check_debug_log("decoder.1.mlp.fc1: Feature=FakeQuantFp8, API=process_tensor: dgrad, gradient: QDQ")

        # process_tensor
        nvinspect_api.transformer_engine.process_tensor("decoder.1.mlp.fc1", gemm="fprop", tensor=tensor, tensor_name="activation")
        #assert check_debug_log("decoder.1.mlp.fc1: Feature=FakeQuantFp8, API=process_tensor: fprop, activation: QDQ")

        nvinspect_api.transformer_engine.process_tensor("decoder.1.mlp.fc1", gemm="dgrad", tensor=tensor, tensor_name="gradient")
        #assert check_debug_log("decoder.1.mlp.fc1: Feature=FakeQuantFp8, API=process_tensor: dgrad, gradient: QDQ")
        ...
        nvinspect_api.transformer_engine.process_tensor("decoder.1.mlp.fc1", gemm="fprop", tensor=tensor, tensor_name="weight")
        nvinspect_api.transformer_engine.process_tensor("decoder.1.mlp.fc1", gemm="dgrad", tensor=tensor, tensor_name="weight")
        torch.testing.assert_close(tensor, nvinspect_api.transformer_engine.process_tensor("decoder.1.mlp.fc1", gemm="wgrad", tensor=tensor, tensor_name="gradient"))
        ...
        nvinspect_api.transformer_engine.process_tensor("decoder.1.mlp.moe.fc1", gemm="fprop", tensor=tensor, tensor_name="activation")
...
def test_feature_medley(configs_dir, feature_dirs):
    ...
    try:
        ...
        default_quantizer = Float8Quantizer(scale=torch.tensor([1]).cuda(), amax=torch.tensor([0]).cuda(), fp8_dtype=tex.DType.kFloat8E4M3)
        nvinspect_api.transformer_engine.process_tensor("decoder.1.attn.qkv", gemm="dgrad", tensor=tensor, tensor_name="gradient", default_quantizer=default_quantizer)
        ...
        nvinspect_api.transformer_engine.process_tensor("decoder.1.mlp.fc1", gemm="wgrad", tensor=tensor, tensor_name="gradient")
        ...
        # Disabling FP8
        torch.testing.assert_close(tensor, nvinspect_api.transformer_engine.process_tensor("decoder.1.mlp.fc1", gemm="fprop", tensor=tensor, tensor_name="activation"))
        nvinspect_api.transformer_engine.process_tensor("decoder.1.mlp.fc1", gemm="wgrad", tensor=tensor, tensor_name="gradient")
...
def test_statistics_collection(configs_dir, feature_dirs):
    ...
    tensor_fp8 = Float8Tensor(data=tensor.to(torch.uint8), fp8_scale_inv=torch.ones([1]).cuda, 
    ...
    # TE tensor stats -- 
    nvinspect_api.transformer_engine.look_at_tensor_before_process("decoder.1.mlp.fc1", tensor=tensor, tensor_name="activation", iteration=200)
    stats = log()
    assert stats[("decoder.1.mlp.fc1", "activation", "cur_amax", 200)] == tensor.abs().max()

    nvinspect_api.transformer_engine.look_at_tensor_before_process("decoder.1.mlp.fc1", tensor=tensor, tensor_name="activation", iteration=201)
    assert_empty()
    nvinspect_api.transformer_engine.look_at_tensor_before_process("decoder.2.mlp.fc1", tensor=tensor, tensor_name="activation", iteration=200)
    assert_empty()
    nvinspect_api.transformer_engine.look_at_tensor_before_process("decoder.1.mlp.fc1", tensor=tensor, tensor_name="gradient", iteration=200)
    assert_empty()
    ...
    assert_empty()
    nvinspect_api.transformer_engine.look_at_tensor_after_process("decoder.2.mlp.fc1", tensor=tensor_fp8, tensor_name="gradient", iteration=200)
    assert_empty()


    # Second config in same yaml
    tensor = torch.rand((100, 100, 5))
    nvinspect_api.transformer_engine.look_at_tensor_before_process("decoder.6.mlp.fc1", tensor=tensor, tensor_name="activation", iteration=200)
    stats = log()
    ...
    nvinspect_api.transformer_engine.look_at_tensor_before_process("decoder.7.mlp.fc1", tensor=tensor, tensor_name="weight", iteration=200)
    stats = log()
    stats_names = [x[3] for x in stats.keys()]
    ...
    nvinspect_api.transformer_engine.look_at_tensor_before_process("decoder.7.mlp.fc1", tensor=tensor, tensor_name="weight", iteration=201)
    assert_empty()
...
def test_statistics_multi_run(configs_dir, feature_dirs):
    ...
    def feed(tensor, tensor_fp8):
        nvinspect_api.transformer_engine.look_at_tensor_before_process("decoder.5.mlp.fc1", tensor=tensor, tensor_name="activation", iteration=1)
