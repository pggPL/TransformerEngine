...
def _emulate_linear(
    ...
    if fprop_fp8:
        activation = _fp8_gemm_kernel(
            input, _scalar(fprop_input_scale or 1.), tex.DType.kFloat8E4M3, 
            weight, _scalar(fprop_weight_scale or 1.), tex.DType.kFloat8E4M3, _2X_ACC_FPROP)
        activation = activation.clone().detach().contiguous().requires_grad_(True)
    else:
        fprop_input = _fake_cast(input, fprop_input_fake_quant, _scalar(fprop_input_scale)) \
            if fprop_input_fake_quant is not None else input
        fprop_weight = _fake_cast(weight, fprop_weight_fake_quant, _scalar(fprop_weight_scale)) \
            if fprop_weight_fake_quant is not None else weight
    ...
    if dgrad_fp8:
        dgrad = _fp8_gemm_kernel(
            weight.T,  _scalar(dgrad_weight_scale or 1.), tex.DType.kFloat8E4M3, 
            gradient,  _scalar(dgrad_gradient_scale or 1.), tex.DType.kFloat8E5M2, _2X_ACC_DGRAD).T
    else:
        dgrad_gradient = _fake_cast(gradient, dgrad_gradient_fake_quant, _scalar(dgrad_gradient_scale)) \
            if dgrad_gradient_fake_quant is not None else gradient
        
        dgrad_weight = _fake_cast(weight, dgrad_weight_fake_quant, _scalar(dgrad_weight_scale)) \
            if dgrad_weight_fake_quant is not None else weight
        dgrad = dgrad_gradient @ dgrad_weight

    if wgrad_fp8:
        wgrad = _fp8_gemm_kernel(
            input.T, _scalar(wgrad_input_scale or 1.), tex.DType.kFloat8E4M3, 
            gradient.T, _scalar(wgrad_gradient_scale or 1.), tex.DType.kFloat8E5M2, _2X_ACC_WGRAD).T
    else:
        wgrad_gradient = _fake_cast(gradient, wgrad_gradient_fake_quant, _scalar(wgrad_gradient_scale)) \
            if wgrad_gradient_fake_quant is not None else gradient
        wgrad_input = _fake_cast(input, wgrad_input_fake_quant, _scalar(wgrad_input_scale)) \
            if wgrad_input_fake_quant is not None else input
...
def test_per_tensor_scaling(feature_dirs, fprop_inp, fprop_weight, dgrad_weight, dgrad_grad, wgrad_input, wgrad_grad):
    ...
    run_per_tensor_scaling(feature_dirs, fprop_inp, fprop_weight, dgrad_weight, dgrad_grad, wgrad_input, wgrad_grad)
...
def _prepare_per_tensor_scaling_config(fprop_inp, fprop_weight, dgrad_weight, dgrad_grad, wgrad_input, wgrad_grad, config_file):
    gemms = ""
...
@create_config_file
def run_per_tensor_scaling(feature_dirs, fprop_inp, fprop_weight, dgrad_weight, dgrad_grad, wgrad_input, wgrad_grad, **kwargs):
    input_kwargs = {
        "fprop_inp": fprop_inp,
...
@pytest.mark.parametrize("fprop_inp, fprop_weight, dgrad_weight, dgrad_grad, wgrad_input, wgrad_grad", subset_combinations)
def test_microbatching_per_tensor_scaling(feature_dirs, fprop_inp, fprop_weight, dgrad_weight, dgrad_grad, wgrad_input, wgrad_grad):
    if not any([fprop_inp, fprop_weight, dgrad_weight, dgrad_grad, wgrad_input, wgrad_grad]):
        pytest.skip("Skipping test because all parameters are False")
    
    @create_config_file
    def run_microbatching_test(feature_dirs, fprop_inp, fprop_weight, dgrad_weight, dgrad_grad, wgrad_input, wgrad_grad, **kwargs):
        # Prepare the configuration file
        _prepare_per_tensor_scaling_config(
            fprop_inp, fprop_weight, dgrad_weight, dgrad_grad, wgrad_input, wgrad_grad, kwargs['config_file']
        )
        ...
        # Compare outputs
        torch.testing.assert_close(output1["activation"], output2["activation"], atol=1., rtol=0.5)
        torch.testing.assert_close(output1["dgrad"], output2["dgrad"], atol=1., rtol=0.5)
        torch.testing.assert_close(output1["wgrad"], output2["wgrad"], atol=1., rtol=0.5)
...
all_combinations = list(itertools.product([tex.DType.kFloat8E4M3, tex.DType.kFloat8E5M2, None], repeat=6))
subset_combinations = random.sample(all_combinations, 10)

@pytest.mark.parametrize("fprop_inp, fprop_weight, dgrad_weight, dgrad_grad, wgrad_input, wgrad_grad", subset_combinations)
def test_fake_quant_fp8(feature_dirs, fprop_inp, fprop_weight, dgrad_weight, dgrad_grad, wgrad_input, wgrad_grad):
    run_fake_quant_fp8(feature_dirs, fprop_inp, fprop_weight, dgrad_weight, dgrad_grad, wgrad_input, wgrad_grad)
...
$gemms
""")
def fake_quant_fp8_create_config(fprop_inp, fprop_weight, dgrad_weight, dgrad_grad, wgrad_input, wgrad_grad, config_file):
    format_to_str = {
        tex.DType.kFloat8E4M3: "E4M3",
        tex.DType.kFloat8E5M2: "E5M2"
    }
    gemms = ""
...
@create_config_file
def run_fake_quant_fp8(feature_dirs, fprop_inp, fprop_weight, dgrad_weight, dgrad_grad, wgrad_input, wgrad_grad, **kwargs):
    fp8_kwargs = {
