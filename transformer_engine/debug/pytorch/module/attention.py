...
from transformer_engine.debug.pytorch.module import LayerNormLinear, Linear
from transformer_engine.debug.pytorch.module.base import TransformerEngineBaseModule
...
from transformer_engine.pytorch.attention import (
    check_set_window_size, get_distributed_world_size,
    _SplitAlongDim, apply_rotary_pos_emb,
    DotProductAttention, InferenceParams
)
        ...
        if self.attention_type == "self":
            ...
            if self.input_layernorm:
                layernorm_qkv_outputs = self.layernorm_qkv(
                    ...
                    overwrite_name=self.name + ".layernorm_linear_qkv"
                )
                ...
                mixed_x_layer = self.qkv(
                    ...
                    overwrite_name=self.name + ".linear_qkv"
                )
            ...
            mixed_kv_layer = self.key_value(
                ...
                is_first_microbatch=is_first_microbatch,
                is_first_module_in_mha=True,  # specific to FP8 MHA
                overwrite_name=self.name + ".linear_kv"
            )
            ...
            if self.input_layernorm:
                layernorm_query_outputs = self.layernorm_query(
                    hidden_states,
                    is_first_microbatch=is_first_microbatch,
                    overwrite_name=self.name + ".layernorm_linear_q"
                )
                if self.return_layernorm_output:
                ...
                query_layer = self.query_layer(
                    ...
                    is_first_microbatch=is_first_microbatch,
                    is_first_module_in_mha=True,  # specific to FP8 MHA
                    overwrite_name=self.name + ".linear_q"
                )
