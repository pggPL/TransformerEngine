...
from ....pytorch.module._common import _ParameterInitMeta
from ....pytorch.export import is_in_onnx_export_mode
from ....pytorch.fp8 import (
...
from ...debug_state import DebugLayerState
...
class TransformerEngineBaseModule(torch.nn.Module, ABC):
    ...
    def set_fp8_weights(self) -> None:
        ...
        names like `weight1_fp8` and `weight1_fp8_t`, cache the FP8
        ...
        for i, shape in enumerate(self.fp8_weight_shapes, start=1):
            weight_fp8_attr = f"weight{i}_fp8"
            weight_fp8_t_attr = f"weight{i}_fp8_t"

            if (
                hasattr(self, weight_fp8_attr)
                and getattr(self, weight_fp8_attr).shape == shape
            ):
                return
            ...
            setattr(
                ...
                Float8Tensor(
                    ...
                    fp8_scale_inv=torch.tensor([1], device=torch.cuda.current_device()),
                )
            )
            setattr(
                self,
    ...
    def init_fp8_metadata(self, num_gemms: int = 1) -> None:
        ...
        self.fp8 = nvinspect_api.transformer_engine.is_fp8_enabled_for_layer(self.name, fp8_enabled=FP8GlobalStateManager.is_fp8_enabled())["ret"]
        self.fp8_calibration = FP8GlobalStateManager.is_fp8_calibration()
        if self.fp8_calibration:
            nvinspect_api.log_message("> FP8 calibration is not supported in the debug module. "
                                "Using this flag will not affect the debug module. ", 
                                level=logging.WARNING)
        ...
        if self.fp8:
            ...
            assert self.fp8_meta["recipe"].fp8_mha == False, "MultiHead Attention in FP8 not supported in debug module."
            self.fp8_meta["num_gemms"] = num_gemms
            self.fp8_meta["fp8_group"] = FP8GlobalStateManager.get_fp8_group()

            if self.fp8_meta["recipe"].override_linear_precision.wgrad:
                nvinspect_api.log_message("> Disabling FP8 Wgrad through --fp8-wgrad argument is not supported in debug mode. "
                                    "Use debug config. ", 
                                    level=logging.WARNING)
    ...
    def prepare_forward(
        ...
        if self.fp8 and in_fp8_activation_recompute_phase():
            print("Activation recomputation not supported in debug mode.")
        else:
            ...
            if FP8GlobalStateManager.fp8_graph_capturing():
                print("Cuda graph not supported")
    ...
    def grad_output_preprocess(
        ...
        if (
            gather_grad_output
            and (ctx.fp8_meta["recipe"].override_linear_precision.wgrad or \
            not enable_fp8_gemm(ctx.layer_cfg, "wgrad", ctx.fp8))
        ):
        ...
        # FP8 case with gather: unfused bgrad, cast, transpose for efficient gather
            ...
            # always return in BF16 for BF16 Dgrad and logging.
            grad_output_mat, _ = gather_along_first_dim(grad_output_mat, ctx.tp_group)
        ...
        if ctx.use_bias:
            ...
            if not ctx.fp8_meta["recipe"].override_linear_precision.wgrad and \
                enable_fp8_gemm(ctx.layer_cfg, "wgrad", ctx.fp8):
                if isinstance(grad_output_mat, Float8Tensor):
                    grad_output_c = grad_output_mat
                    grad_output_t = grad_output_c.transpose_2d()
                else:
    ...
    def _validate_name(self, overwrite_name: str = None):
        """
