...
from transformer_engine.debug.pytorch.module import LayerNormMLP, MultiheadAttention
from transformer_engine.debug.pytorch.module.base import TransformerEngineBaseModule
from transformer_engine.pytorch.module import LayerNorm, RMSNorm
...
class TransformerLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        self.self_attention = MultiheadAttention(
            ...
            **common_attention_kwargs,
            ...
            device=device,
            name=(self.name + ".self_attn" if self.name is not None else None)
        )
        ...
        if layer_type == "decoder":
            self.inter_attention = MultiheadAttention(
                ...
                **common_attention_kwargs,
                ...
                normalization=normalization,
                device=device,
                name=(self.name + ".inter_attn" if self.name is not None else None)
            )
    ...
    def forward(
        ...
        self_attention_outputs = self.self_attention(
            ...
            overwrite_name=self.name + ".self_attn"
        )
        ...
        if self.layer_type == "decoder":
            inter_attention_outputs = self.inter_attention(
                ...
                alibi_slopes=alibi_slopes,
                fast_zero_fill=fast_zero_fill,
                overwrite_name=self.name + ".inter_attn"
            )
        ...
        mlp_outputs = self.layernorm_mlp(
            hidden_states,
            is_first_microbatch=is_first_microbatch,
            overwrite_name=self.name + ".layernorm_mlp"
        )
        if self.apply_residual_connection_post_layernorm:
