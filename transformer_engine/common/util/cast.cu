...
struct Numeric_Traits<fp8e5m2> {
  ...
  static constexpr double maxSubnorm =
      0.75 / static_cast<double>(1 << 14);                               // std::pow(2.0, -14.0);
...
struct Numeric_Traits<fp32> {
  static constexpr double minSubnorm =
      std::numeric_limits<fp32>::denorm_min();  // std::pow(2.0, -149.0);
  static constexpr double maxSubnorm =
      std::numeric_limits<fp32>::min() -
  ...
  static constexpr double maxNorm =
      std::numeric_limits<fp32>::max();  // (1 - pow(2, -24)) * pow(2, 128)
...
struct Quantized_Limits {
  static constexpr double ranges[] = {0.0, Numeric_Traits<T>::minNorm, Numeric_Traits<T>::maxNorm,
  ...
  static constexpr inline int max_norm_biased_exponent() {
    return Numeric_Traits<T>::maxBiasedExponentAsFP32;
  }
  static constexpr inline int max_norm_unbiased_exponent() {
    return Numeric_Traits<T>::maxUnbiasedExponentAsFP32;
  }
};
...
__global__ void __launch_bounds__(THREADS_PER_CHUNK)
    cast_mxfp8_kernel(const __grid_constant__ CUtensorMap tensor_map_input,
                      const __grid_constant__ CUtensorMap tensor_map_output, e8m0_t *const scales,
...
#pragma unroll
      for (int prefetch_stage = 0; prefetch_stage < PREFETCH_STAGES; ++prefetch_stage) {
        ...
        cp_async_bulk_tensor_2d_global_to_shared(
            ...
            chunk_stage_offset_y, &mbar[prefetch_stage]);
...
#pragma unroll
      ...
      const CType block_scale_inverse =
...
#pragma unroll
      ...
      if (is_master_thread) {
        ...
        cp_async_bulk_tensor_2d_shared_to_global(
            ...
            chunk_it_offset_y, reinterpret_cast<uint64_t *>(&out_sh[stage]));
...
void cast_mxfp8(const Tensor &input, Tensor *output_, cudaStream_t stream) {
  ...
  TRANSFORMER_ENGINE_TYPE_SWITCH_INPUT(
      ...
      TRANSFORMER_ENGINE_TYPE_SWITCH_FP8ONLY(
          output.data.dtype, OType, CUtensorMap tensor_map_input{}; CUtensorMap tensor_map_output{};
          ...
          cast_mxfp8_kernel<fp32, IType, OType><<<grid, block, 0, stream>>>(
              tensor_map_input, tensor_map_output, scales_ptr, rows, cols););  // NOLINT(*)
  );                                                                           // NOLINT(*)
