...
cast_mxfp8_kernel(const __grid_constant__ CUtensorMap tensor_map_input,
    ...
    constexpr size_t THREADS_PER_CHUNK_Y_ROWWISE = THREADS_PER_CHUNK
                                                   / THREADS_PER_CHUNK_X_ROWWISE;   //  16 = 64 / 4
            ...
            if constexpr (IS_ROWWISE_SCALING) {
                ...
                for (int stage = 0; stage < BUFF_STAGES_NUM; ++stage) {
                    ...
                    if (tid_rowwise_X % THREADS_PER_SCALE_X_ROWWISE == 0) {
                        const int global_scales_offset_Y = iteration_scale_rowwise_offset_Y + stage_offset_Y + tid_rowwise_Y;
                        const int global_scales_offset_X = scales_rowwise_chunk_offset_X + tid_rowwise_X / THREADS_PER_SCALE_X_ROWWISE;
                        const int scale_idx = global_scales_offset_Y * scale_stride_rowwise + global_scales_offset_X;
                        scales_rowwise[scale_idx] = biased_exponent;
                    }

                    const float block_scale_inverse = exp2f(FP32_EXPONENT_BIAS - static_cast<float>(biased_exponent));
                    #pragma unroll
                    for (int j = 0; j < ELEMS_PER_THREAD; ++j) {
...
void cast_mxfp8(const Tensor& input, Tensor* output_, cudaStream_t stream) {
    ...
    switch (SCALE_DIM) {                                                    \
        case  1: { constexpr size_t DIM =  1; { __VA_ARGS__ } } break;      \
        case 32: { constexpr size_t DIM = 32; { __VA_ARGS__ } } break;      \
        default: { NVTE_ERROR("Invalid size of the MX scaling factor."); }  \
    }

    SCALE_DIM_SWITCH(scale_dim_Y, SCALE_DIM_Y,
      SCALE_DIM_SWITCH(scale_dim_X, SCALE_DIM_X,
        TRANSFORMER_ENGINE_TYPE_SWITCH_INPUT(input.data.dtype, IType,
          TRANSFORMER_ENGINE_TYPE_SWITCH_FP8ONLY(output.data.dtype, OType,
            ...
            cast_mxfp8_kernel<IS_DBIAS, IS_DACT, ParamOP, OP, IType, OType, SCALE_DIM_Y, SCALE_DIM_X>
                <<<grid, block, 0, stream>>>
                (tensor_map_input, tensor_map_output, tensor_map_output,
                 scales_ptr_rowwise, scales_ptr_colwise, rows, cols,
      ...
      SCALE_DIM_SWITCH(scale_dim_X_rowwise, SCALE_DIM_X,
        TRANSFORMER_ENGINE_TYPE_SWITCH_INPUT(input.data.dtype, IType,
          TRANSFORMER_ENGINE_TYPE_SWITCH_FP8ONLY(OutputType, OType,
            ...
            create_tensor_map<IType>(tensor_map_input, input,
                                     rows, cols, shmem_dim_Y, shmem_dim_X);
            create_tensor_map<OType>(tensor_map_output_rowwise, output_rowwise,
                                     rows, cols, shmem_dim_Y, shmem_dim_X);
            create_tensor_map<OType>(tensor_map_output_colwise, output_colwise,
                                     rows, cols, shmem_dim_Y, shmem_dim_X);
