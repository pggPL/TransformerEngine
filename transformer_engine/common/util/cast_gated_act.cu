...
__global__ void __launch_bounds__(THREADS_PER_CHUNK)
  ...
  const uint64_t dshmem_aligned_as_uint = DIVUP(dshmem_unaligned_as_uint, static_cast<uint64_t>(ALIGNMENT_SIZE)) * ALIGNMENT_SIZE;
  ...
  if (is_master_thread) {
    ...
    ptx::cp_async_bulk_tensor_2d_global_to_shared(
        reinterpret_cast<uint64_t *>(&in_grad_sh[0]),
        TMAP_grad_in, chunk_offset_X, chunk_offset_Y, &mbar[0]);

    // Act
    ptx::cp_async_bulk_tensor_2d_global_to_shared(
        reinterpret_cast<uint64_t *>(&in_act_sh[0]),
        TMAP_gate_in, chunk_offset_X, chunk_offset_Y, &mbar[0]);

    // Gate
    ptx::cp_async_bulk_tensor_2d_global_to_shared(
        reinterpret_cast<uint64_t *>(&in_gate_sh[0]), 
        TMAP_gate_in, chunk_offset_X + cols, chunk_offset_Y, &mbar[0]);
  ...
  for (int it = 0; it < ITERATIONS; ++it) {
    ...
    if (next_it < ITERATIONS) {
      if (is_master_thread) {
        ...
        // Grad
        ptx::cp_async_bulk_tensor_2d_global_to_shared(
            reinterpret_cast<uint64_t *>(&in_grad_sh[next_buff * buff_elems]),
            TMAP_grad_in, chunk_it_offset_x, chunk_it_offset_y, &mbar[next_it]);
        // Act
        ptx::cp_async_bulk_tensor_2d_global_to_shared(
            reinterpret_cast<uint64_t *>(&in_act_sh[next_buff * buff_elems]),
            TMAP_gate_in, chunk_it_offset_x, chunk_it_offset_y, &mbar[next_it]);
        // Gate
        ptx::cp_async_bulk_tensor_2d_global_to_shared(
            reinterpret_cast<uint64_t *>(&in_gate_sh[next_buff * buff_elems]),
            TMAP_gate_in, chunk_it_offset_x + cols, chunk_it_offset_y, &mbar[next_it]);
...
__global__ void __launch_bounds__(THREADS_PER_CHUNK)
  ...
  const uint64_t *TMAP_grad_in = reinterpret_cast<const uint64_t *>(&tensor_map_grad);
  const uint64_t *TMAP_gate_in = reinterpret_cast<const uint64_t *>(&tensor_map_gated_input);
  const uint64_t *TMAP_output_rowwise = reinterpret_cast<const uint64_t *>(&tensor_map_output_rowwise);
  const uint64_t *TMAP_output_colwise = reinterpret_cast<const uint64_t *>(&tensor_map_output_colwise);
  ...
  for (int it = 0; it < ITERATIONS; ++it) {
    ...
    for (int stage = 0; stage < BUFFER_STAGES_NUM; ++stage) {
      ...
      const float amax_gated_elem = fmaxf(fabsf(after_dact_reg[stage]), fabsf(after_dgate_reg[stage]));
      ...
      if constexpr (USE_ROWWISE_SCALING) {
        ...
        const e8m0_t biased_exponent_X = float_to_e8m0(mx_block_X_amax * Quantized_Limits<OType>::max_norm_rcp);
        const float scale_reciprocal_X = exp2f_rcp(biased_exponent_X);

        out_act_rowwise_sh_curr[shmem_idx] = static_cast<OType>(scale_reciprocal_X * after_dact_reg[stage]);
        out_gate_rowwise_sh_curr[shmem_idx] = static_cast<OType>(scale_reciprocal_X * after_dgate_reg[stage]);

        // Only single thread writes the computed scaling factor
        if (tid_X % SCALE_DIM_X == 0) {
          const int global_scales_offset_Y = iteration_scale_rowwise_offset_Y + stage_offset_Y + thread_offset_Y;
    ...
    if constexpr (USE_COLWISE_SCALING) {
      ...
      const e8m0_t biased_exponent = float_to_e8m0(mx_block_Y_amax * Quantized_Limits<OType>::max_norm_rcp);
      ...
      for (int stage = 0; stage < BUFFER_STAGES_NUM; ++stage) {
        ...
        out_act_colwise_sh_curr[shmem_idx] = static_cast<OType>(scale_reciprocal * after_dact_reg[stage]);
        out_gate_colwise_sh_curr[shmem_idx] = static_cast<OType>(scale_reciprocal * after_dgate_reg[stage]);
      }
...
void cast_fp8_dgated(const Tensor &grad, const Tensor &gated_input, Tensor *output,
  ...
  TRANSFORMER_ENGINE_TYPE_SWITCH_INPUT(grad.data.dtype, IType,
      TRANSFORMER_ENGINE_TYPE_SWITCH_FP8ONLY(output->data.dtype, OType,
          ...
          create_2D_tensor_map<IType>(tensor_map_grad, &grad, rows, cols,
                                      SHMEM_DIM_Y, SHMEM_DIM_X);
          create_2D_tensor_map<IType>(tensor_map_gated_input, &gated_input, rows, cols * 2,
                                      SHMEM_DIM_Y, SHMEM_DIM_X);
          create_2D_tensor_map<OType>(tensor_map_output, output, rows, cols * 2,
                                      SHMEM_DIM_Y, SHMEM_DIM_X);
          ...
          cast_fp8_dgated_kernel<ParamOP, ActOP, DActOP, IType, OType>
              ...
              (tensor_map_grad, tensor_map_gated_input, tensor_map_output, amax_ptr, scale_inv_ptr,
               scale_ptr, rows, cols);
      );  // NOLINT(*)
  );      // NOLINT(*)
}
...
void cast_mxfp8_dgated(const Tensor &grad, const Tensor &gated_input, Tensor *output_rowwise,
  ...
  constexpr bool USE_ROWWISE_SCALING = (SCALING == ScalingType::BIDIMENTIONAL || SCALING == ScalingType::ROWWISE);
  constexpr bool USE_COLWISE_SCALING = (SCALING == ScalingType::BIDIMENTIONAL || SCALING == ScalingType::COLWISE);
  ...
  float *const amax_ptr_rowwise = USE_ROWWISE_SCALING
                                  ? reinterpret_cast<float *>(output_rowwise->amax.dptr)
                                  : nullptr;
  float *const amax_ptr_colwise = USE_COLWISE_SCALING
                                  ? reinterpret_cast<float *>(output_colwise->amax.dptr)
                                  : nullptr;
      ...
      TRANSFORMER_ENGINE_MX_SCALE_DIM_SWITCH(scale_dim_X_rowwise, SCALE_DIM_X,
          TRANSFORMER_ENGINE_TYPE_SWITCH_INPUT(grad.data.dtype, IType,
              TRANSFORMER_ENGINE_TYPE_SWITCH_FP8ONLY(OutputType, OType,
                  ...
                  create_2D_tensor_map<IType>(tensor_map_grad, &grad,
                                              rows, cols, SHMEM_DIM_Y, SHMEM_DIM_X);
                  create_2D_tensor_map<IType>(tensor_map_gated_input, &gated_input,
                                              rows, cols * 2, SHMEM_DIM_Y, SHMEM_DIM_X);

                  if constexpr (USE_ROWWISE_SCALING) {
                    create_2D_tensor_map<OType>(tensor_map_output_rowwise, output_rowwise,
                                                rows, cols * 2, SHMEM_DIM_Y, SHMEM_DIM_X);
                  }
                  
                  if constexpr (USE_COLWISE_SCALING) {
                    create_2D_tensor_map<OType>(tensor_map_output_colwise, output_colwise,
                                                rows, cols * 2, SHMEM_DIM_Y, SHMEM_DIM_X);
                  }
                  ...
                  cudaFuncSetAttribute(cast_mxfp8_dgated_kernel<ParamOP, ActOP, DActOP, IType, OType, SCALE_DIM_Y, SCALE_DIM_X>,
                                       cudaFuncAttributeMaxDynamicSharedMemorySize, shmem_size);

                  cast_mxfp8_dgated_kernel<ParamOP, ActOP, DActOP, IType, OType, SCALE_DIM_Y, SCALE_DIM_X>
                      <<<grid_dim, block_dim, shmem_size, stream>>>
                      ...
                      amax_ptr_rowwise, amax_ptr_colwise, rows, cols,
...
void fp8_quantize_dgated(const Tensor &grad, const Tensor &gated_input, Tensor *output,
  ...
  if (is_fp8_cast_supported_shape(output)) {
    ...
    if (is_colwise_scaling) {
      cast_mxfp8_dgated<ScalingType::COLWISE, ParamOP, ActOP, DActOP>
          (grad, gated_input, nullptr, output, stream);
    } else {
      cast_mxfp8_dgated<ScalingType::ROWWISE, ParamOP, ActOP, DActOP>
          (grad, gated_input, output, nullptr, stream);
    }
  } else {
...
void nvte_fp8_quantize_swiglu_x2(const NVTETensor grad, const NVTETensor gated_input,
  ...
  fp8_quantize_dgated_x2<Empty, Activation, dActivation>
      (*reinterpret_cast<const Tensor *>(grad), *reinterpret_cast<const Tensor *>(gated_input),
      reinterpret_cast<Tensor *>(output_rowwise), reinterpret_cast<Tensor *>(output_colwise), stream);
}
