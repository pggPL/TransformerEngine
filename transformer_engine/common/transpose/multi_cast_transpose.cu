...
void multi_cast_transpose(const std::vector<Tensor*> input_list, std::vector<Tensor*> output_list,
  ...
  for (size_t tensor_id = 0; tensor_id < input_list.size(); ++tensor_id) {
    ...
    NVTE_CHECK(input.data.shape.size() == 2,
               "Input tensor must have 2 dimensions, but shape is ", input.data.shape);
    NVTE_CHECK(output.data.shape == input.data.shape,
               "C output tensor shape ", output.data.shape,
               "does not match input tensor shape ", input.data.shape);
    NVTE_CHECK(output.columnwise_data.shape.size() == 2,
               "T output tensor shape ", output.columnwise_data.shape,
               "does not match input tensor shape ", input.data.shape);
