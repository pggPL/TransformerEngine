...
class _moe_permute(torch.autograd.Function):
    ...
    def forward(
        ...
        if fp8:
            assert len(inp._quantizer.scale) == 1, "Only one factor scaling per tensor (Delayed Scaling) supported by moe_permute."
        ...
        if fp8:
            permuted_act = Float8Tensor(
                data=permuted_act, fp8_dtype=dtype, fp8_scale_inv=fp8_scale_inv, shape=permuted_act.shape, dtype=fake_dtype
            )
        ...
        if ctx.needs_input_grad[0]:
            ...
            if ctx.fp8:
                act_grad = Float8Tensor(
                    data=act_grad, fp8_dtype=dtype, fp8_scale_inv=fp8_scale_inv * ctx.topK, shape=act_grad.shape, dtype=fake_dtype
                )
...
class _moe_unpermute(torch.autograd.Function):
    ...
    def forward(
        ...
        if fp8:
            unpermuted_output = Float8Tensor(
                ...
                shape=unpermuted_output.shape, dtype=fake_dtype
            )
    ...
    def backward(
        ...
        if ctx.needs_input_grad[0]:
            ...
            if ctx.fp8:
                act_grad = Float8Tensor(
                    data=act_grad, fp8_dtype=dtype, fp8_scale_inv=fp8_scale_inv,
