...
namespace {
// Helper: build an int64 dim vector from a stable tensor's shape.
...
torch::stable::Tensor fused_rope_forward(const torch::stable::Tensor &input,
                                         const torch::stable::Tensor &freqs,
                                         const std::optional<torch::stable::Tensor> start_positions,
                                         const NVTE_QKV_Format qkv_format, const bool interleaved,
                                         const std::optional<torch::stable::Tensor> cu_seqlens,
  ...
  STD_TORCH_CHECK(freqs.scalar_type() == torch::headeronly::ScalarType::Float,
                  "Dtype of the freqs tensor must be float");

  // output (same dtype/device as input)
  auto output = torch::stable::new_empty(input, shape_vec(input));
  ...
  if (qkv_format == NVTE_QKV_Format::NVTE_THD) {
    ...
    STD_TORCH_CHECK(input.size(2) >= freqs.size(3),
    ...
    nvte_fused_rope_forward(input_cu.data(), cu_seqlens_cu.data(), freqs_cu.data(),
                            ...
                            cp_size, cp_rank, max_s, b, h, d, d2, stride_t, /*stride_b=*/0, stride_h,
                            stride_d, getCurrentCUDAStream());
...
fused_qkv_rope_forward(const torch::stable::Tensor &qkv_input, const torch::stable::Tensor &q_freqs,
                       const torch::stable::Tensor &k_freqs,
                       const std::optional<torch::stable::Tensor> start_positions,
                       const std::vector<int> &qkv_split_arg_list, const NVTE_QKV_Format qkv_format,
                       const bool interleaved, const int cp_size, const int cp_rank) {
  STD_TORCH_CHECK(q_freqs.dim() == 4, "expected 4D tensor");
  STD_TORCH_CHECK(q_freqs.size(1) == 1 && q_freqs.size(2) == 1,
                  "expected the second and third dims of the freqs tensor equal 1");
  STD_TORCH_CHECK(q_freqs.scalar_type() == torch::headeronly::ScalarType::Float,
                  "Dtype of the freqs tensor must be float");
  STD_TORCH_CHECK(k_freqs.dim() == 4, "expected 4D tensor");
  STD_TORCH_CHECK(k_freqs.size(1) == 1 && k_freqs.size(2) == 1,
                  "expected the second and third dims of the freqs tensor equal 1");
  STD_TORCH_CHECK(k_freqs.scalar_type() == torch::headeronly::ScalarType::Float,
                  "Dtype of the freqs tensor must be float");
  // output
  auto q_out_size = shape_vec(qkv_input);
  q_out_size[2] = q_out_size[2] * qkv_split_arg_list[0] / qkv_split_arg_list[1];
  q_out_size[3] = qkv_split_arg_list[1];
  auto q_out = torch::stable::new_empty(qkv_input, q_out_size);
  auto k_out_size = shape_vec(qkv_input);
  k_out_size[3] = qkv_split_arg_list[1];
  auto k_out = torch::stable::new_empty(qkv_input, k_out_size);
  auto v_out_size = shape_vec(qkv_input);
  v_out_size[3] = qkv_split_arg_list[2];
  auto v_out = torch::stable::new_empty(qkv_input, v_out_size);
  ...
  nvte_fused_qkv_rope_forward(qkv_cu.data(), q_freqs_cu.data(), k_freqs_cu.data(),
                              ...
                              v_out_cu.data(), qkv_format, interleaved, cp_size, cp_rank, s, b, h, d,
                              d2, qkv_split_arg_list[0], qkv_split_arg_list[1], qkv_split_arg_list[2],
                              getCurrentCUDAStream());

  return std::make_tuple(q_out, k_out, v_out);
}

torch::stable::Tensor fused_rope_backward(const torch::stable::Tensor &output_grads,
                                          const torch::stable::Tensor &freqs,
                                          const std::optional<torch::stable::Tensor> start_positions,
                                          const NVTE_QKV_Format qkv_format, const bool interleaved,
                                          const std::optional<torch::stable::Tensor> cu_seqlens,
  ...
  auto input_grads = torch::stable::new_empty(output_grads, shape_vec(output_grads));
  ...
  if (qkv_format == NVTE_QKV_Format::NVTE_THD) {
    ...
    STD_TORCH_CHECK(cu_seqlens.has_value(), "expected cu_seqlens tensor");
    STD_TORCH_CHECK(cu_seqlens.value().dim() == 1, "expected 1D tensor");
    STD_TORCH_CHECK(output_grads.size(2) >= freqs.size(3),
    ...
    nvte_fused_rope_backward(output_grads_cu.data(), cu_seqlens_cu.data(), freqs_cu.data(),
                             ...
                             /*stride_b=*/0, stride_h, stride_d, getCurrentCUDAStream());
  ...
  auto cu_seqlens_cu = TensorWrapper();  // empty cu_seqlens tensor
  nvte_fused_rope_backward(output_grads_cu.data(), cu_seqlens_cu.data(), freqs_cu.data(),
                           start_positions_cu.data(), input_grads_cu.data(), qkv_format, interleaved,
                           cp_size, cp_rank, s, b, h, d, d2, stride_s, stride_b, stride_h, stride_d,
                           getCurrentCUDAStream());
...
torch::stable::Tensor fused_qkv_rope_backward(
    const torch::stable::Tensor &q_grad_out, const torch::stable::Tensor &k_grad_out,
    const torch::stable::Tensor &v_grad_out, const torch::stable::Tensor &q_freqs,
    const torch::stable::Tensor &k_freqs, const std::vector<int> &qkv_split_arg_list,
    const NVTE_QKV_Format qkv_format, const bool interleaved, const int cp_size, const int cp_rank) {
  auto qkv_grad_size = shape_vec(q_grad_out);
  auto total_hd =
  ...
  auto qkv_grad_input = torch::stable::new_empty(q_grad_out, qkv_grad_size);
  const bool is_sbhd = qkv_format == NVTE_QKV_Format::NVTE_SBHD;
  ...
  nvte_fused_qkv_rope_backward(q_grad_out_cu.data(), k_grad_out_cu.data(), v_grad_out_cu.data(),
                               ...
                               qkv_split_arg_list[1], qkv_split_arg_list[2], getCurrentCUDAStream());
