...
void fp8_block_scaling_compute_partial_amax(const Tensor &tensor, Tensor amax, size_t h,
  ...
  nvte_fp8_block_scaling_compute_partial_amax(tensor_cu.data(), amax_cu.data(), h, w,
                                              ...
                                              block_len, getCurrentCUDAStream());
}
...
void fp8_block_scaling_partial_cast(const Tensor &inp, Tensor out, const Tensor &scale,
  ...
  TORCH_CHECK(out.scalar_type() == ScalarType::Byte, "output must be a uint8 tensor");
  ...
  nvte_fp8_block_scaling_partial_cast(
      ...
      start_offset, block_len, static_cast<NVTEDType>(out_dtype), getCurrentCUDAStream());
}
