...
void multi_tensor_adam_param_remainder_cuda(
    int chunk_size, torch::stable::Tensor noop_flag,
    std::vector<std::vector<torch::stable::Tensor>> tensor_lists, const float lr, const float beta1,
    const float beta2, const float epsilon, const int step, const int mode,
    const int bias_correction, const float weight_decay) {
  auto noop_flag_cu = makeTransformerEngineTensor(noop_flag);
  auto [_, __, tensor_lists_ptr, num_lists, num_tensors] =
      makeTransformerEngineTensorList(tensor_lists);
...
void multi_tensor_adam_capturable_cuda(
    int chunk_size, torch::stable::Tensor noop_flag,
    std::vector<std::vector<torch::stable::Tensor>> tensor_lists, torch::stable::Tensor lr,
    const float beta1, const float beta2, const float epsilon, torch::stable::Tensor step,
