...
void multi_tensor_compute_scale_and_scale_inv_cuda(
    ...
    std::vector<std::vector<torch::stable::Tensor>> tensor_lists, float max_fp8,
  ...
  nvte_multi_tensor_compute_scale_and_scale_inv_cuda(
      ...
      force_pow_2_scales, epsilon, getCurrentCUDAStream());
}

void multi_tensor_compute_scale_inv_e8m0_cuda(
    int chunk_size, const nb::object &dummy,
  ...
  nvte_multi_tensor_compute_scale_inv_e8m0_cuda(chunk_size, tensor_lists_ptr.data(), num_lists,
                                                num_tensors, getCurrentCUDAStream());
}
