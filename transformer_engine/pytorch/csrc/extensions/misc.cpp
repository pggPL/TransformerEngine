...
Tensor splits_to_offsets(const Tensor &first_dims, int64_t logical_last_dim) {
  ...
  NVTE_CHECK(first_dims.scalar_type() == kLong, "first_dims must have dtype int64.");
  ...
  auto output = empty({static_cast<int64_t>(num_tensors) + 1},
                          first_dims_contiguous.options().dtype(kLong));
...
std::tuple<Tensor, std::vector<Tensor>> splits_to_offsets_multi(
  ...
  const Tensor split_sizes_i64 =
      split_sizes.scalar_type() == kLong ? split_sizes : split_sizes.to(kLong);
  const Tensor split_sizes_out =
  ...
  if (bulk_allocate_outputs) {
    ...
    for (size_t i = 0; i < num_outputs; ++i) {
      ...
      outputs.emplace_back(
          empty({length}, TensorOptions().dtype(dtypes[i]).device(device)));
    }
  }
...
Tensor copy_data_ptrs_to_device(const std::vector<Tensor> &tensors,
  ...
  auto ptrs_device = empty({static_cast<int64_t>(tensors.size())},
