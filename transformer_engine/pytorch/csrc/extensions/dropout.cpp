...
std::vector<py::object> dropout_fwd(const py::handle &input, float dropout_probability,
  ...
  if (!out) {
    ScalarType dtype = GetATenDType(input_nvte.dtype());
    if (dtype == kFloat8_e4m3fn || dtype == kFloat8_e5m2) {
      dtype = input.attr("dtype").cast<ScalarType>();
    }
    ...
    const auto opts = TensorOptions().dtype(dtype).device(kCUDA);
    out = empty(shape_int64, opts);
  }
  ...
  NVTE_SCOPED_GIL_RELEASE({
    nvte_extract_seed_and_offset(
        ...
        philox_args.offset_.val, philox_args.offset_intragraph_, getCurrentCUDAStream());
  });
  ...
  NVTE_SCOPED_GIL_RELEASE({
    nvte_dropout_fwd(input_nvte.data(), out_nvte.data(), mask_nvte.data(), rng_state_nvte.data(),
                     dropout_probability, getCurrentCUDAStream());
  });
